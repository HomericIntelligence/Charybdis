#!/bin/bash
# Run the Charybdis CI suite locally inside a container.
#
...
#
# Image: uses 'charybdis-ci:local' if available, falls back to GHCR image.
# Build locally: just ci-build  (or: podman build -f ci/Containerfile -t charybdis-ci:local .)
...
LOCAL_IMAGE="charybdis-ci:local"
